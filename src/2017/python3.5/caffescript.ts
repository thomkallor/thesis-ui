...
RUN bin/bash -c 'source activate caffe && cd /usr/local/caffe && mkdir build && cd build && \
	cmake -DPYTHON_INCLUDE_DIR=/usr/local/miniconda${'${PYTHON}'}/envs/caffe/include/python${'${PYTHON_VERSION}'}m \
	-DPYTHON_INCLUDE_DIR2=/usr/local/miniconda${'${PYTHON}'}/envs/caffe/include/python${'${PYTHON_VERSION}'}m \
	-DPYTHON_LIBRARY=/usr/local/miniconda${'${PYTHON}'}/envs/caffe/lib/libpython${'${PYTHON_VERSION}'}m.so \
